...
function CarCallouts() {
  const callouts = [
    ...
    { label: "High-Performance Powertrain", x: "55%", y: "40%", align: "center" },
        ...
        >
          <div className="flex flex-col items-center">
            ...
            <div className="mt-4 px-3 py-1 bg-black/40 backdrop-blur-md border border-white/10 rounded-lg whitespace-nowrap">
              <span className="text-[10px] uppercase font-bold tracking-widest text-white">{c.label}</span>
            </div>
          </div>
        </motion.div>
      ))}
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      for (let i = 1; i < 4; i++) {
        ...
        const currentRadius = Math.sqrt(Math.pow(radius, 2) - Math.pow(cy - y, 2));
        ctx.ellipse(cx, y, currentRadius, currentRadius * 0.2, 0, 0, Math.PI * 2);
        ctx.stroke();
      }
...
function RightHUD() {
  return (
    <div className="absolute top-[20%] right-12 flex flex-col gap-6 w-64 pointer-events-auto">
      <HUDCard title="Customization" icon={<Settings className="w-3 h-3 text-accent" />}>
      ...
      <HUDCard title="Wheel" icon={<Cpu className="w-3 h-3 text-accent" />}>
        <div className="p-4">
          <p className="text-white text-lg font-display mb-1">Precision engineered,</p>
          <p className="text-subtle text-sm">curated alloy formulations</p>
        </div>
      </HUDCard>

      <HUDCard title="Performance" icon={<Zap className="w-3 h-3 text-accent" />}>
        <div className="flex flex-col items-center py-6 gap-2">
          <div className="relative w-32 h-32 flex items-center justify-center">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              ...
              <span className="text-[10px] uppercase text-subtle tracking-widest font-bold">OUTPUT</span>
            </div>
          </div>
        </div>
      </HUDCard>
...
function HUDCard({ title, icon, children }: { title: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-3xl overflow-hidden shadow-2xl transition-all duration-500 hover:border-accent/30 group">
      <div className="px-6 py-4 flex justify-between items-center border-b border-white/5">
        <h4 className="text-[10px] uppercase tracking-[0.2em] text-subtle font-bold group-hover:text-accent transition-colors">{title}</h4>
        {icon}
      </div>
      {children}
    </div>
  ...
  return (
    <div className="relative group flex flex-col items-center gap-1 transition-transform hover:scale-110 active:scale-95 cursor-pointer">
      <Icon className={cn(
        ...
        active ? "text-accent drop-shadow-[0_0_8px_#C7A43D]" : "text-subtle group-hover:text-white"
      )} />
      {active && (
        <motion.div 
